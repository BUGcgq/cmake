 ...
 * @Date: 2023-05-05 14:36:50
 * @LastEditors: LIYAOHAN 1791002655@qq.com
 * @LastEditTime: 2023-05-06 10:22:44
 * @FilePath: /OCPP/ocpp_ftp.c
...
static int ocpp_ftp_itoa(int value, char * str, int radix);
static int ocpp_ftp_send_cmd(int ctrl_sock,enum OCPP_FTP_CMD_TYPE_E type, const char *val,const char *ack_num);
static int ocpp_ftp_enter_passive_mode(int ctrl_sock,char *data_ip, int * data_port);
static int ocpp_ftp_enter_active_mode(int ctrl_sock);

 
static int ocpp_ftp_get_data_sock(const char* server_ip,const int port);
static int ocpp_ftp_get_active_data_sock(int client_server_sock);
 
static int ocpp_ftp_GetAddr(const char *ifname, char *addr, int flag);
static int ocpp_ftp_close_st_info(FTP_DATA_INFO * info);
...
int ocpp_ftp_connect_server(const char* server_ip,const int port){
    ...
    server.sin_addr.s_addr = ((struct in_addr *)server_ip)->s_addr;//inet_addr(server_ip); 
    ...
    //成功
    if(strncmp(read_buf,ACK_CONNECT_NUM,3) == 0) {
...
int ocpp_ftp_login_server(int ctrl_sock,const char *user_name, const char * passwd){
    ...
    ret = ocpp_ftp_send_cmd(ctrl_sock,CMD_USER,user_name,ACK_USER_NUM);
    if(ret < 0){
    ...
    ret = ocpp_ftp_send_cmd(ctrl_sock,CMD_PASS,passwd,ACK_PASS_NUM);
    if(ret < 0){
        printf("send_cmd  %d failed \n",CMD_PASS);
        return -1;
    }
...
static int ocpp_ftp_send_cmd(int ctrl_sock,enum OCPP_FTP_CMD_TYPE_E type, const char *val,const char *ack_num){
    ...
    if((type == CMD_USER) ||(type == CMD_PASS) || (type == CMD_CWD)){
...
static int ocpp_ftp_enter_active_mode(int ctrl_sock){
    ...
    */
    ...
    ret = ocpp_ftp_send_cmd(ctrl_sock,CMD_PORT_FTP, send_buf,ACK_PORT_NUM);
    if(ret < 0){
...
static int ocpp_ftp_enter_passive_mode(int ctrl_sock,char *data_ip, int * data_port){
    ...
    ret = ocpp_ftp_send_cmd(ctrl_sock,CMD_PASV,NULL,NULL);
    if(ret < 0){
            printf("send_cmd  %d failed \n",CMD_PASV);
            return -1;
    }
    ...
    if(strstr(read_buf,ACK_PASV_NUM) != NULL){
        ...
        ocpp_ftp_itoa(ip1,tmp_buf,10);
        strcat(data_ip,tmp_buf);
        strcat(data_ip,".");
        
        memset(tmp_buf,0,sizeof(tmp_buf));
        ocpp_ftp_itoa(ip2,tmp_buf,10);
        strcat(data_ip,tmp_buf);
        strcat(data_ip,".");
 
        memset(tmp_buf,0,sizeof(tmp_buf));
        ocpp_ftp_itoa(ip3,tmp_buf,10);
        strcat(data_ip,tmp_buf);
        strcat(data_ip,".");
 
        memset(tmp_buf,0,sizeof(tmp_buf));
        ocpp_ftp_itoa(ip4,tmp_buf,10);
        strcat(data_ip,tmp_buf);
        //printf("data_ip1 = %s\n",data_ip);
...
int ocpp_ftp_down_file(int ctrl_sock, char *server_filepath_name,const  char *newfilename,int connect_mode,int offset,enum OCPP_FTP_CMD_TYPE_E type){
    ...
    ocpp_ftp_close_st_info(&server_info);
    ...
    if((type !=CMD_RETR) && (type !=CMD_LIST)){
    ...
    if(connect_mode){ /*主动模式*/
        ...
        server_info.client_server_sock = ocpp_ftp_enter_active_mode(ctrl_sock);
        ...
        ret = ocpp_ftp_enter_passive_mode(ctrl_sock,server_info.data_ip, &server_info.data_port);
        if(ret < 0){
                printf("set passive mode failed\n");
                return -1;
        }
        printf("server_info.data_ip =%s, data_port =%d \n",server_info.data_ip,server_info.data_port);
        server_info.data_sock = ocpp_ftp_get_data_sock(server_info.data_ip,server_info.data_port);
    ...
    /*更改目录*/
      ...
      {
         ret =  ocpp_ftp_send_cmd(ctrl_sock,CMD_CWD,server_info.server_path,ACK_CWD_NUM);
         if(ret < 0){
                printf("set passive mode failed\n");
                goto err0;
         }
     ...
     if(type ==CMD_RETR){/*发送下载文件命令*/
 
         /*设置下载文件偏移的位置*/
         if(offset >0){
        
             flags =O_CREAT|O_RDWR|O_APPEND;     
             ocpp_ftp_itoa(offset,stri,10);
             ret =  ocpp_ftp_send_cmd(ctrl_sock,CMD_REST,stri,ACK_REST_NUM);
              if(ret < 0){
                     printf("set file offsize failed\n");
                     goto err0;
              }  
         }
         ret = ocpp_ftp_send_cmd(ctrl_sock,CMD_RETR,server_info.server_filename,ACK_RETR_NUM);
         if(ret < 0){
         ...
         ret = ocpp_ftp_send_cmd(ctrl_sock,CMD_LIST,server_filepath_name,ACK_LIST_NUM);
         if(ret < 0){
    ...
    if(connect_mode){
        server_info.data_sock= ocpp_ftp_get_active_data_sock(server_info.client_server_sock);
        if(server_info.data_sock <0)
        {
            printf("accept failed\n");
            goto err0;
...
int ocpp_ftp_up_file(int ctrl_sock, char *server_filepath_name,const  char *srcfilename,int connect_mode,int offset){
    ...
    /*发送上传文件命令*/
     ret = ocpp_ftp_send_cmd(ctrl_sock,CMD_STOR,server_info.server_filename,ACK_STOR_NUM);
     if(ret < 0){
 ...
 * @description: 
 * @param {int} ctrl_sock
 * @param {char} *server_filepath_name
 * @return {*}
 */ 
int ocpp_ftp_get_fsize(int ctrl_sock, char *server_filepath_name){
    ...
    //发送上传文件命令
    ret = ocpp_ftp_send_cmd(ctrl_sock,CMD_SIZE_FTP,server_filepath_name,ACK_SIZE_NUM);
    if(ret < 0){
...
int ocpp_ftp_quit_server(int ctrl_sock){
    ...
    ret = ocpp_ftp_send_cmd(ctrl_sock,CMD_QUIT,NULL,ACK_QUIT_NUM);
    if(ret < 0)
